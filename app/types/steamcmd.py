...
# Wine/Windows-depot SteamCMD servers (Enshrouded) KEEP the tmux launch: their
# run command carries inline `WINEPREFIX=... wine ...` env that `exec` can't
# parse, and they aren't part of the native world-load regression. configure()
# selects this template when the recipe is a Wine game.
...
class SteamCmdHandler(TypeHandler):
    ...
    def configure(self) -> list[str]:
        ...
        # Native SteamCMD games launch DIRECTLY under systemd (no tmux) — see
        # _START_TEMPLATE. Wine games keep the tmux launch (their inline
        # WINEPREFIX=... env can't be exec'd, and they're outside the native
        # world-load regression we're fixing).
        if uses_wine:
            _start_tmpl, _stop_tmpl, _style = _START_TEMPLATE_WINE, _STOP_TEMPLATE_WINE, "tmux (wine)"
        else:
            _start_tmpl, _stop_tmpl, _style = _START_TEMPLATE, _STOP_TEMPLATE, "direct-exec (no tmux)"
        self.write_script(
            "start.sh",
