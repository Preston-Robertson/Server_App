...
# Launch strategy for NATIVE (non-Wine) SteamCMD games — see configure().
#   "direct" — exec the game as the systemd unit's MainPID (no tmux): RAM is the
#              real cgroup number; stop = SIGINT to gamesrv.pid.
#   "tmux"   — run inside a private tmux server (the original launch).
# Wine games always use tmux regardless of this toggle. Set to "tmux" on
# 2026-07-11 to A/B test connectivity: the user could CONNECT to Palworld under
# the "direct" launch DESPITE the manager still showing "starting" at ~1 GB RAM.
# We're testing whether tmux ALSO accepts connections — if it does, the real bug
# is the readiness probe / RAM expectation, not tmux-vs-direct. Flip back to
# "direct" to restore the no-tmux launch.
_NATIVE_LAUNCH = "tmux"


_START_TEMPLATE = """#!/usr/bin/env bash
# Generated by gamesrv manager. Runs a NATIVE-Linux SteamCMD dedicated server
# DIRECTLY as this systemd unit's main process — NO tmux — matching the
# known-good BitsNBytes native deploy (PalServer.sh straight from ExecStart).
#
# WHY NO TMUX (2026-07-11): after every launch-ENVIRONMENT fix (NOFILE, keyctl,
# SENTRY, NFS, memory, HOME, XDG_RUNTIME_DIR) failed to un-stall Palworld's
# world load at ~1 GB, tmux was the LAST structural delta vs a working deploy
# AND the one constant across the whole regression — Palworld loaded fine
# INSIDE Minecraft's tmux server but stalled in its OWN. Running the game on a
# tmux pseudo-terminal one step removed from the unit cgroup is now eliminated
# as a variable. Bonus: the game becomes the true MainPID => accurate cgroup
# memory (kills the old "848 KB" tmux-escape bug), native `systemctl stop`, and
# real exit codes. Native SteamCMD servers expose no interactive stdin console
# (they use RCON/REST), so dropping tmux send-keys costs nothing; graceful stop
# is SIGINT (see stop.sh).
set -euo pipefail
cd "$(dirname "$0")"

# Raise the open-file SOFT limit to the HARD ceiling. Unreal dedicated servers
# open tens of thousands of fds streaming world .pak/.ucas assets on boot; a low
# soft limit (systemd default ~1024) silently STALLS world load at ~1 GB. Robust
# regardless of the unit's LimitNOFILE.
...
# Steam's networking/IPC layer keeps its control sockets under $XDG_RUNTIME_DIR.
# A non-login systemd user has no /run/user/<uid>, so point it at a real tmpfs
# (NEVER an on-disk/NFS path — that stalls Steam IPC). Prefer /run/user/<uid>
# (the known-good value), else /dev/shm, else /tmp. Export BEFORE exec so the
# game inherits it.
_uid="$(id -u)"
...
# One-time migration off the OLD tmux launch: kill any stale session for THIS
# server (shared default socket AND its private socket) so a leftover instance
# isn't still holding the game port when we exec. Harmless once migrated.
if command -v tmux >/dev/null 2>&1; then
  tmux kill-session -t "gs-{name}" 2>/dev/null || true
  tmux -L "gs-{name}" kill-session -t "gs-{name}" 2>/dev/null || true
fi

# Preserve accumulated console across restarts, but cap runaway size (>10 MB).
...
{{ echo ""; echo "===== $(date -Iseconds) start.sh launching (gs-{name}, direct-exec) ====="; }} >> console.log
...
# Run the game as the unit's MainPID. `exec` REPLACES this shell, so the game
# KEEPS this PID (== systemd MainPID for Type=simple). Record it first so
# stop.sh can send a clean SIGINT (it equals $$ because exec preserves the PID).
# Redirect output to console.log — the manager's Console tab reads it (same file
# the old tmux pipe-pane wrote); the unit journal keeps start.sh's marker lines.
echo $$ > gamesrv.pid
exec $BIN >> "$(pwd)/console.log" 2>&1
"""


# TMUX launch template. Used for ALL Wine games (their inline
# `WINEPREFIX=... wine ...` env can't be exec'd) AND for native games while
# _NATIVE_LAUNCH == "tmux" (the current rollback default — see configure()).
_START_TEMPLATE_WINE = """#!/usr/bin/env bash
...
# Graceful stop for a DIRECT-exec native server (start.sh: the game is this
# unit's MainPID, and its PID is in gamesrv.pid because exec preserved it).
# Send SIGINT — Palworld/UE dedicated servers treat it as a clean shutdown
# (flush the world save, exit code 130) — then wait for it to actually exit.
# systemd escalates to SIGKILL only past the unit's TimeoutStopSec (300s), so
# we wait just under that.
set -euo pipefail
cd "$(dirname "$0")"
pid=""
[[ -f gamesrv.pid ]] && pid="$(cat gamesrv.pid 2>/dev/null || echo)"
if [[ -z "${{pid:-}}" ]] || ! kill -0 "$pid" 2>/dev/null; then
  exit 0
fi
kill -INT "$pid" 2>/dev/null || true
for _ in $(seq 1 280); do
  kill -0 "$pid" 2>/dev/null || {{ rm -f gamesrv.pid 2>/dev/null || true; exit 0; }}
  sleep 1
done
echo "stop.sh: game pid $pid did not exit within 280s; systemd will SIGKILL" >&2
"""


# Wine games keep the tmux-based graceful stop (see _START_TEMPLATE_WINE).
_STOP_TEMPLATE_WINE = """#!/usr/bin/env bash
...
class SteamCmdHandler(TypeHandler):
    ...
    def configure(self) -> list[str]:
        ...
        # Launch strategy. Wine games ALWAYS use the tmux template
        # (_START_TEMPLATE_WINE) — their inline WINEPREFIX=... env can't be
        # exec'd. Native games follow the _NATIVE_LAUNCH toggle at the top of
        # this module:
        #   "direct" -> _START_TEMPLATE      (exec as systemd MainPID, no tmux)
        #   "tmux"   -> _START_TEMPLATE_WINE (same tmux launch as wine)
        # 2026-07-11: toggled to "tmux" to A/B test connectivity — the user could
        # connect under direct-exec despite a ~1 GB RAM reading, so we're
        # checking whether tmux ALSO connects (=> real bug is readiness/RAM
        # reporting, not tmux). Flip _NATIVE_LAUNCH to "direct" to restore.
        if uses_wine or _NATIVE_LAUNCH == "tmux":
            _start_tmpl, _stop_tmpl = _START_TEMPLATE_WINE, _STOP_TEMPLATE_WINE
            _style = "tmux (wine)" if uses_wine else "tmux (native rollback)"
        else:
            _start_tmpl, _stop_tmpl = _START_TEMPLATE, _STOP_TEMPLATE
            _style = "direct-exec (no tmux)"
        self.write_script(
            "start.sh",
            _start_tmpl.format(name=self.sd.name, run_cmd=run_cmd),
        )
        self.write_script("stop.sh", _stop_tmpl.format(name=self.sd.name))
        msgs.append(f"wrote {self.install_dir/'start.sh'} [{_style}]")
